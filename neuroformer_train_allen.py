...
from neuroformer.datasets import load_visnav, load_V1AL, load_allen
...
if args.dataset in ["lateral", "medial"]:
    ...
    stimulus_type = args.stim
    data, datasets, callback = load_allen(container_id, stimulus_type)
...
if stimulus_type == "natural_scenes":
    ...
    stimulus = center_crop(resize(torch.tensor(stimulus)))
elif stimulus_type == "locally_sparse_noise":
    resize = Resize((30, 100))
    stimulus = resize(torch.tensor(stimulus))
elif stimulus_type == "static_gratings":
    max_idx = max(stimulus.keys())
    stimulus_ = torch.zeros((max_idx + 1, 30, 100))
    center_crop = CenterCrop((30, 100))
    stimulus = {k: center_crop(torch.tensor(stim)) for k, stim in stimulus.items()}
    for k, stim in stimulus.items():
        stimulus_[k] = stim
    stimulus = stimulus_
elif stimulus_type == "drifting_gratings":
    max_idx = max(stimulus.keys())
    stimulus_ = torch.zeros((max_idx + 1, 60, 30, 100))
    center_crop = CenterCrop((30, 100))
    stimulus = {k: center_crop(torch.tensor(stim)) for k, stim in stimulus.items()}
    for k, stim in stimulus.items():
        stimulus_[k] = stim
...
datasets['train'].stimuli = stimulus
datasets['test'].stimuli = stimulus
...
CKPT_PATH = f"./models/NF.15/Visnav_VR_Expt/{args.dataset}/{model_name}_{container_id}_{stimulus_type}"
